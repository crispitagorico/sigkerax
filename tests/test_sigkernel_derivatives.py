...
class TestSigKernelDerivatives:
  ...
  def test_derivatives_along_zero_direction(self):
    ...
    assert jnp.all(jnp.isclose(k_mat[..., 1:], 0.0, atol=1e-12))

  def test_derivatives_along_constant_direction(self):
    """Derivatives along the zero path is always equal to 0"""
    X = 5e-1 * jax.random.normal(getkey(), shape=(self.batch_X, self.length_X, self.dim), dtype=self.dtype)
    Y = 5e-1 * jax.random.normal(getkey(), shape=(self.batch_Y, self.length_Y, self.dim), dtype=self.dtype)
    directions1 = 5e-1 * jax.random.normal(getkey(), shape=(2, self.batch_X, self.dim), dtype=self.dtype)
    directions1 = jnp.tile(directions1[:, :, jnp.newaxis, :], (1, 1, self.length_X, 1))
    directions2 = 5e-1 * jax.random.normal(getkey(), shape=(2, self.batch_X, self.dim), dtype=self.dtype)
    ...
    assert jnp.all(jnp.isclose(k_mat1[..., 1:], k_mat2[..., 1:], atol=1e-12))
  ...
  def test_derivative_padded_direction(self):
    ...
    directions = jax.random.normal(getkey(), shape=(2, self.batch_X, self.length_X, self.dim), dtype=self.dtype)
  ...
  def test_derivatives_perturbative(self, eps=1e-4):
    ...
    k_eps = (1./eps) * (self.signature_kernel.kernel_matrix(X + eps * directions[0], Y) - self.signature_kernel.kernel_matrix(X, Y))[..., 0]
    ...
    print(jnp.mean(jnp.abs(k2_diff - k2_eps)))
    assert jnp.allclose(k_diff, k_eps, atol=1e-6) & jnp.allclose(k2_diff, k2_eps, atol=1e-1)