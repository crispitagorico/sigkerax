...
class TestSigKernel:
  ...
  batch_X, batch_Y, length_X, length_Y, dim = 5, 5, 20, 20, 5
  ...
  def test_linear_paths(self):
    ...
    X = jax.random.normal(getkey(), shape=(self.batch_X, 2, self.dim), dtype=self.dtype)
    Y = jax.random.normal(getkey(), shape=(self.batch_Y, 2, self.dim), dtype=self.dtype)
    k_mat = self.signature_kernel.kernel_matrix(X, Y)
    X_inc = X[:, 1, :] - X[:, 0, :]
    Y_inc = Y[:, 1, :] - Y[:, 0, :]
    ip_inc_mat = jnp.einsum('ik,jk->ij', X_inc, Y_inc)
    k_mat_bessel = I0(4.0 * ip_inc_mat, num_terms=30, dtype=self.dtype)
